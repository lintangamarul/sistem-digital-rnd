            ...
            <div class="table-responsive mb-3">
              ...
              <thead>
                  <tr class="text-center">
                      ...
                      <th width="6%">Qty</th>
                      <th width="15%">Estimasi Ukuran Part<br>(p x l) (mm)</th>
                      ...
                      '<div style="width:100%;">' +
                        ...
                        '<div style="display:flex; margin-bottom:5px; width:100%;">' +
                          '<input type="number" name="dies['+ index +'][panjang]" placeholder="p" class="form-control" ' +
                                'style="width:48%; margin-right:4%;" oninput="calculateKeliling(this)">' +
                          '<input type="number" name="dies['+ index +'][lebar]" placeholder="l" class="form-control" ' +
                                'style="width:48%;" oninput="calculateKeliling(this)">' +
                        '</div>' +
...
function onMachineChange(elem) {
  ...
  function calculateKeliling(element) {
    var row = element.closest('tr');
    var index = row.getAttribute('data-index'); // Get the row index from data attribute

    // Get inputs using the correct name format with index
    var panjang = row.querySelector(`input[name="dies[${index}][panjang]"]`).value;
    var lebar = row.querySelector(`input[name="dies[${index}][lebar]"]`).value;
    var panjangProses = row.querySelector(`input[name="dies[${index}][panjangProses]"]`).value;
    var diameter = row.querySelector(`input[name="dies[${index}][diameter]"]`).value;
    var jumlahPie = row.querySelector(`input[name="dies[${index}][jumlahPie]"]`).value;

    // Parse values, default to 0 if empty/NaN
    ...
    var keliling = 0;
    const isPanjangProsesValid = panjangProsesValue > 0;
    const isPieValid = diameterValue > 0 && jumlahPieValue > 0;

    if (isPanjangProsesValid && isPieValid) {
        // Kombinasi panjangProses + PIE
        keliling = (panjangProsesValue) + (3.14 * diameterValue * jumlahPieValue);
    } else if (isPanjangProsesValid) {
        // Hanya panjangProses
...
function togglePieInputsEditable(row) {
    ...
    // Set conditions based on process combinations
    if (hasPie && hasBendTrimSepRest) {
        // PIE + (BEND/TRIM/SEP/REST): enable all
        inputDiameter.readOnly = false;
        inputJumlahPie.readOnly = false;
        inputPanjangProses.readOnly = false;
    } else if (hasPie) {
        // Only PIE: enable diameter and jumlahPie
        inputDiameter.readOnly = false;
        inputJumlahPie.readOnly = false;
    } else if (hasBendTrimSepRest) {
        // Only BEND/TRIM/SEP/REST: enable panjang proses
        inputPanjangProses.readOnly = false;
    }
}
