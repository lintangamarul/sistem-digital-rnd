          ...
          <div class="card p-3 mb-3">
            ...
            <div class="row">
              ...
              <div class="col-md-4 mb-3">
                ...
                <select name="material" id="materialDropdown" class="form-control custom-select2 mb-2" style="width: 100%; height: 38px;" onchange="  calculateKelilingForAllRows();">
                    ...
                    <?php foreach ($dies as $index => $die): ?>
                      <tr data-index="<?= $index ?>">
                        ...
                        <td>
                          ...
                          <input type="hidden" name="dies[<?= $index ?>][cbPanjangLebar]" value="0">
                            <input
                              type="checkbox"
                            ...
                            <div style="margin-bottom: 5px; width: 100%; display: flex; align-items: center; gap: 6px;">
                              <input type="hidden" name="dies[<?= $index ?>][cbPanjangProses]" value="0">
                              <input
                                type="checkbox"
                            ...
                            <div style="display: flex; width: 100%; align-items: center; gap: 6px;">
                              <input type="hidden" name="dies[<?= $index ?>][cbPie]" value="0">
                              <input
                                type="checkbox"
...
//         if (lengthMpInput) {
//             calculateKeliling(lengthMpInput);
//         } else if (panjangInput) {
//             calculateKeliling(panjangInput);
//         }
//     });
// }
...
// Fungsi helper untuk memanggil calculateKeliling pada semua baris
function calculateKelilingForAllRows() {
    // Cari semua baris dalam tabel
    const tableRows = document.querySelectorAll('tbody tr[data-index]');
     console.log(tableRows + "calculatekelil");
    tableRows.forEach(function(row) {
        // Ambil input pertama dari kolom keliling untuk dijadikan parameter
        // Bisa menggunakan input panjang, lebar, atau length_mp sebagai parameter
        const lengthMpInput = row.querySelector('input[name*="[length_mp]"]');
        const panjangInput = row.querySelector('input[name*="[panjang]"]');
        
        // Gunakan salah satu input sebagai parameter untuk calculateKeliling
        if (lengthMpInput) {
...
function calculateKelilingForRow(rowIndex) {
    const row = document.querySelector(`tr[data-index="${rowIndex}"]`);
    if (row) {
        const lengthMpInput = row.querySelector(`input[name="dies[${rowIndex}][length_mp]"]`);
        if (lengthMpInput) {
            calculateMainPressure(lengthMpInput);
        }
    }
...
// Solusi 4: Event handler yang lebih robust
function initializeMaterialDropdown() {
    ...
    if (materialDropdown) {
        materialDropdown.addEventListener('change', function() {
            ...
            tableRows.forEach(function(row) {
                const triggerElement = row.querySelector('input[name*="[length_mp]"]') || 
                                     row.querySelector('input[name*="[panjang]"]');
                
                if (triggerElement) {
                  console.log(triggerElement);
                    calculateMainPressure(triggerElement);
                }
            });
        });
    }
...
// Solusi 5: Jika menggunakan jQuery (jika sudah ada di project)
/*
$(document).ready(function() {
    $('#materialDropdown').on('change', function() {
        $('tbody tr[data-index]').each(function() {
            var lengthMpInput = $(this).find('input[name*="[length_mp]"]')[0];
            if (lengthMpInput) {
                calculateKeliling(lengthMpInput);
            }
        });
