...
.piket-list::-webkit-scrollbar-thumb {
  ...
  gap: 15px 15px; /* horizontal gap tetap 15px, vertical gap dikurangi */
    row-gap: 1px; /* Jarak vertikal diperkecil drastis dari 8px ke 3px */
  height: calc(100vh - 180px); /* Kurangi height untuk menghindari scroll */
  min-height: 700px;
}
    .grid-item {
      display: flex;
...
<div class="grid-item">
  <div class="piket-container">
    ...
    <div class="piket-wrapper">
      ...
      <!-- Kolom Piket Besok -->
      <div class="piket-column">
        <div class="piket-header text-info">Besok</div>
        <div class="piket-list">
          <?php if (!empty($piketTomorrow)): ?>
            <ul>
              <?php foreach ($piketTomorrow as $nama): ?>
                <li><?= $nama ?></li>
              <?php endforeach; ?>
            </ul>
          <?php else: ?>
            <div class="no-piket">Tidak ada jadwal piket besok</div>
          <?php endif; ?>
        </div>
      </div>
    </div>
